...
def prof_perform(
    ...
    if two_nodes:
        assert world_size==16 and causal==False
        ...
        if log_all:
            for iter0 in range(warm_ups,test_nums+warm_ups,1):
                if accuracy_test:
                    ...
                    # log(f"ring lse diff {iter0}", local_lse.unsqueeze(dim=-1) - dist_lse[iter0])
                    cosine_sim_out=F.cosine_similarity(torch.flatten(local_out),torch.flatten(dist_out[iter0]),dim=0)
...
if __name__ == "__main__":
    ...
    print(f"Rank {global_rank}: finish {len(completed_tasks)} task, continue")
