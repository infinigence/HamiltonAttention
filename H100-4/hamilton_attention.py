...
def gen_hamilton_circle_two_nodes(n):
    assert (n==16 or n==32), "currently n==16/32 is supported"
    res=[]
    ...
    if n==16:
        ...
        for row in p8:
            res.append(row+[x+8 for x in row]+[x+16 for x in row]+[x+24 for x in row])
        return res
...
def calculate_inter_node_mapping(looping):
    ...
    assert (n==16 and m==8) or (n==32 and m==8), "currently n==16/32 m==8 is supported"
...
def extract_local_for_zigzag_ring(x_all, rank, world_size, dim=1):
    ## ##
    # use dim=2 for lse use dim = 1 for others
    ## ##
