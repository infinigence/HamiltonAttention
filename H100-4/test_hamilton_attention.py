...
def prof_perform(
    ...
    if two_nodes:
        assert (world_size==16 or world_size==32) and causal==False
    two_nodes=(two_nodes and (world_size==16 or world_size==32)  and causal==False)                 
    ...
    if not two_nodes:
        ...
        looping=gen_hamilton_circle_two_nodes(node_num*8)
...
if __name__ == "__main__":
    ...
    print(f"Rank {global_rank}: #all-task {len(all_tasks)} #pending-task {len(pending_tasks)}")
    ...
    for i, (seqlen0, bs0, nhead, d, task_sig) in enumerate(pending_tasks):
        ...
        try:
            ...
            print(f"Rank {global_rank}: finish and saved task")
